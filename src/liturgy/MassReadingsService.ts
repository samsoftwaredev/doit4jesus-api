...
function normalizeInput(input: GetMassReadingsInput): GetMassReadingsInput {
  ...
  if (country && !/^[A-Z]{2}$/.test(country)) {
    throw new ApiError(400, 'INVALID_COUNTRY', 'country must use a two-letter ISO country code.');
  }

  const diocese = input.diocese?.trim().toLowerCase();
  if (diocese && !/^[a-z0-9]+(?:[ -][a-z0-9]+)*$/.test(diocese)) {
    throw new ApiError(400, 'INVALID_DIOCESE', 'diocese must contain letters, numbers, spaces, or hyphens only.');
  }
  ...
  if (locale) {
    try {
      ...
      throw new ApiError(400, 'INVALID_LOCALE', 'locale must be a valid BCP 47 language tag.');
    }
  }
...
export function applicationToday(now = new Date(), timezone = process.env.LITURGY_TIMEZONE ?? DEFAULT_TIMEZONE) {
  try {
    ...
    throw new ApiError(500, 'LITURGY_RESOLUTION_ERROR', 'LITURGY_TIMEZONE must be a valid IANA timezone.');
  }
}
...
export class MassReadingsService {
  constructor(
    ...
    private readonly lectionary = new LectionaryResolver(new LocalLectionaryRepository()),
    private readonly cache: MassReadingsCache = new InMemoryMassReadingsCache(),
    private readonly verseMapper = new NabreVerseMapper(),
  ) {}

  async getMassReadings(input: GetMassReadingsInput): Promise<DailyMassReadings> {
    ...
    if (!resolution.primary) {
      throw new ApiError(
        ...
        { date: day.date, celebrationId: day.primaryCelebration.id, dataVersion: DATA_VERSION },
      );
    }

    const readingSets = normalized.includeVerseText !== false
      ? await Promise.all(
    ...
    if (!primary) {
      throw new ApiError(404, 'READINGS_NOT_FOUND', 'No readings are available for this date.');
    }
