-- Timezone-aware rosary completion aggregation. This intentionally computes
-- live values so a user's current timezone is consistently applied to history.

create index if not exists idx_spiritual_activities_rosary_completion
  on competition.spiritual_activities (user_id, completed_at)
